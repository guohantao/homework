...
data segment  
    ...
    out_buf db 20*(student_db+6) dup(' ') ;输出表项时用到        
...
;--------------------------------------------------------------------------------------------------
    ...
    flag1 dw 0 ;区分 二转十进制 显示整数十进制数 -1  还是带小数的十进制数 -0
    flag2 dw 0 ;区分 二转十进制 要不要显示十进制数 - 0，还是不显示，只把现实的数据放在num_buf中-1
    0_60 dw 0
    60_70 dw 0
    70_80 dw 0
    80_90 dw 0
...
print_table proc
   ...
   lp_print_table:
        ...
        ; 姓名
          ...
          pop si   
          ;打印回车换行
          add si,2   ;si指向一个学生单位中的 换行位置 即偏移量为32
          mov cx,2
          rep movsb
        pop cx
      loop lp_print_table 
      mov [di],'$'  ;给out_buf 设置结束符
...
bin_to_dec proc
  ...
  zero: 
    ...
    mov dx,0   ;DX清零,因为要做32位除以16位除法，所以要关注DX的变化 
